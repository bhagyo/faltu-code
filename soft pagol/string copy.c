 ...
 *There are two rule to explain your observation:
 1. Evaluation order of function parameter is unspecified.
 So, you never know which parameter will be excuted first.
 2. modifying the same variable in the same sequence twice or more is undefined.
 So, your program is not conform to C standard hence not even correct.
 */
